...
class NeuralNetworkRegressor(nn.Module):
  ...
  optim={'name': 'Adam', 'learning_rate': 0.01}, scheduler=None, criterion='MSE', random_state=42):
      '''
      ...
      :param optim: optimizer name (see torch.optim) and parameters to set it up
      :param scheduler: argumetns to setup a learning rate scheduler
      :param criterion: loss function ['MSE', 'L1']
      :param random_state: 
      '''
  ...
  def get_optim(self, optim_kwargs: dict) -> optim:
      ...
      if optim_kwargs['name'] == 'AdamW':
        return optim.AdamW(self.model.parameters(), **{k: v for k, v in optim_kwargs.items() if k != 'name'})
  ...
  def get_scheduler(self, scheduler_kwargs):
        """
        Initialize learning rate scheduler.
        """
        if scheduler_kwargs is None:
            return None
        if scheduler_kwargs["name"] is None:
            return None
        if scheduler_kwargs["name"] == 'StepLR': # Reduces LR by gamma every step_size epochs
            return optim.lr_scheduler.StepLR(self.optimizer, **{k: v for k, v in scheduler_kwargs.items() if k != 'name'})
        elif scheduler_kwargs["name"] == 'ReduceLROnPlateau': # Reduces LR when validation loss stops improving
            return optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, **{k: v for k, v in scheduler_kwargs.items() if k != 'name'}) 
        elif scheduler_kwargs["name"] == 'CosineAnnealingLR': # Adjusts LR following a cosine decay schedule
            return optim.lr_scheduler.CosineAnnealingLR(self.optimizer, **{'T_max':50}) ##**{k: v for k, v in scheduler_kwargs.items() if k != 'name'})
        else:
            raise Exception(f'Scheduler {scheduler_kwargs["name"]} not implemented')
  ...
  def fit(self, X: np.array, y: np.array, X_test: np.array, y_test: np.array):
      ...
      for epoch in range(self.epochs):
          ...
          for batch_X, batch_y in train_loader:
              ...
              #weights = 1.0 / (1.0 + batch_y) 
              #loss = (weights * (outputs - batch_y)**2).mean()
              loss = self.criterion(outputs, batch_y)
              """
              # ----- weighted loss -----
              errors = (outputs - batch_y) ** 2       # per-sample MSE
              errors = errors.view(-1)
              alpha = 3.0
              weights = 1 + alpha * torch.exp(-batch_y.view(-1))
          ...
          if self.scheduler:
              if isinstance(self.scheduler, optim.lr_scheduler.ReduceLROnPlateau):
                  self.scheduler.step(test_loss)  # ReduceLROnPlateau needs validation loss
              else:
                  self.scheduler.step()  # Other schedulers step every epoch

          if (epoch + 1) % 10 == 0: # Report on train and test loss every 10 epochs
              print(f'Epoch [{epoch+1}/{self.epochs}], Train Loss: {train_loss:.4f} (RMSE {np.sqrt(train_loss):.4f}), Test Loss: {test_loss:.4f} (RMSE {np.sqrt(test_loss):.4f})')
  ...
  def test_scores(self, y_test: pd.Series, y_pred: np.array, dataset: str, score_path = None):
      ...
      #rmselow = root_mean_squared_error(y_test[y_test<3], y_pred[y_test<3])
