...
def codistribute_vars(lut, lut_config):
    ...
    Sample Car from Cab using relations car = cab/5 + 1
    ...
    if lut_config['codistribution'] is not None:
      ...
      # Sample Car from Cab using relations car = cab/5 + 1
      if 'cab' in lut._samples.columns and 'car' in lut._samples.columns:
          lut._samples['car'] = lut._samples['cab'] / 5.0 + 1.0
...
def generate_spectra(
  ...
  fpath_lut = output_dir.joinpath(f'{pheno_phases}_lut.pkl') 
  print(fpath_lut)
...
def generate_spectra_soil(
  ...
  # generate lookup-table
  fpath_lut = output_dir.joinpath(f'{pheno_phases}_12_lut.pkl') 
  print(fpath_lut)
...
if __name__ == '__main__':
  ...
  fpath_srf = Path(config['RTM']['fpath_srf']) if config['RTM']['fpath_srf'] is not None else None
